...
您的分析核心邏輯結合了以下經典與現代理論：
1. **道氏理論 (Dow Theory)**：確認主要趨勢。
2. **葛蘭碧八大法則 (Granville's Rules)**：判斷均線與股價的乖離與支撐。
3. **指標共振 (Confluence)**：特別關注 KD 與 MACD 的雙重確認。
...
* **產業階段**：(參考 AI 股展望邏輯) 判斷該公司處於「基礎建設期」、「應用爆發期」還是「成熟期」？
* **營收動能**：最近 3 個月營收年增率 (YoY) 趨勢。
* **估值位階**：本益比 (PE) 與 股價淨值比 (PB) 的歷史區間位置。

## 3. 籌碼面詳解 (Chip Analysis)
* **法人動向**：外資與投信近 5 日是連續買超、賣超還是調節？(注意投信作帳行情)。
* **大戶籌碼**：千張大戶持股比例是否增加？籌碼是否流向散戶？

## 4. 技術面深度戰法 (Technical Deep Dive) - *重點精華*
請嚴格依據以下邏輯進行分析：
* **趨勢定義 (道氏理論)**：目前是「高過前高、低不破低」的多頭架構，還是空頭排列？
* **均線戰法 (葛蘭碧)**：股價相對於 MA20/MA60 的位置。是否有「回測均線不破」或「乖離過大」的訊號？
* **指標雙重確認 (KD + MACD)**：
    * 檢查 **KD 指標** 是否出現黃金交叉/死亡交叉？
    * 檢查 **MACD 柱狀體 (Histogram)** 是否翻紅/翻綠？
    * *關鍵判斷*：是否出現「KD 金叉 且 MACD 柱狀體向上」的高勝率共振訊號？
* **布林通道 (Bollinger Bands)**：股價是否沿著上軌運行 (強勢) 或觸及下軌 (超賣)？
* **K線型態**：識別是否有明顯的「吞噬」、「錘頭」、「晨星」或「墓碑」等反轉型態。
...
# 從環境變數讀取 Key (請在 Streamlit Secrets 設定 GOOGLE_API_KEY)
...
with st.sidebar:
    ...
    if not api_key:
        st.warning("⚠️ 請先在 Streamlit Secrets 設定 GOOGLE_API_KEY")
